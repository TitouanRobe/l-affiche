...
set schema 'bddreco';
CREATE VIEW films_genres_notes AS SELECT id_film, id_genre, nom_genre, titre, note, nb_votes, poster from bddreco._film natural join bddreco._note natural join bddreco._genre natural join bddreco._film_genre WHERE CAST(nb_votes AS FLOAT) > 20000 ORDER BY note DESC;
...
ALTER TABLE bddreco._film 
ADD COLUMN poster VARCHAR(300);

ALTER TABLE bddreco._film
ADD COLUMN backdrop VARCHAR(300);
